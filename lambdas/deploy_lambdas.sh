...
# Constants
STACK_NAME="blr-home"
LAYER_NAME_KEYS=("LayerCommonName")
PREFIX="Lambda"
SUFFIX="Name"
    ...
    for i in "${!layer_names[@]}"; do
        ...
        if [[ -n "$layer_name" && -n "$layer_arn" ]]; then
            if [[ "$FORCE_UPDATE" = true || $layer_modified_epoch -gt $lambda_modified_epoch ]]; then
                ...
                echo "Layer $layer_name needs update for lambda $lambda_name"
            else
                layers_to_update+=("$layer_arn")
                echo "Layer $layer_name for lambda $lambda_name is already up-to-date."
            fi
        fi
    done
